    ...
    int VND_Intra_Swap(vector<int>& rota, int custo_anterior){
        ...
        for(int i = 1; i < n-2; i++){
            for(int j = i+1; j < n-1; j++){
                ...
                // O que estariamos removendo
                custo_atual =  p.matriz_custo[rota[i-1]][rota[i]]; 
                custo_atual += p.matriz_custo[rota[i]][rota[i+1]]; 
                custo_atual += p.matriz_custo[rota[j-1]][rota[j]]; 
                custo_atual += p.matriz_custo[rota[j]][rota[j+1]]; 

                // O que estariamos adicionando
                custo_swap =  p.matriz_custo[rota[i-1]][rota[j]];
                custo_swap += p.matriz_custo[rota[j]][rota[i+1]];
                custo_swap += p.matriz_custo[rota[j-1]][rota[i]];
                custo_swap += p.matriz_custo[rota[i]][rota[j+1]];
    ...
    int VND_Intra_ShiftN(vector<int>& rota, int custo_anterior){
        ...
        for(int i = 1; i < rota.size() - 2; i++){
            for(int j = i+2; j < rota.size() - 1; j++){
                ...
                int max_seg = min(nMax, (int) rota.size() - 2 - i);
                if(max_seg < 1) continue;

                int segmento = 1 + rand() % max_seg;
                vector<int> rota_teste = rota;
                rota_teste.insert(rota_teste.begin() + j, rota_teste.begin() + i, rota_teste.begin() + i + segmento);
                rota_teste.erase(rota_teste.begin() + i, rota_teste.begin() + i + segmento);

                custo_teste = CustoDemanda(rota_teste);
    ...
    int VND_Intra_Reinsertion(vector<int>& rota, int custo_anterior){
        ...
        for(int i = 1; i < rota.size() - 1; i++){
            for(int j = 1; j < rota.size() - 1; j++){
                ...
                custo_reinsertion = p.matriz_custo[rota[j-1]][rota[i]];
                custo_reinsertion += p.matriz_custo[rota[i]][rota[j]];
                custo_reinsertion += p.matriz_custo[rota[i-1]][rota[i+1]];

                // O que seria removido
                custo_reinsertion -= p.matriz_custo[rota[i-1]][rota[i]];
                custo_reinsertion -= p.matriz_custo[rota[i]][rota[i+1]];
                custo_reinsertion -= p.matriz_custo[rota[j-1]][rota[j]];

                // Se o custo compensar
                if(custo_reinsertion < 0){
                    ...
                    if(ReinsertionTest(rota, i, j)){
                        ...
                        custo_anterior += custo_reinsertion;
                    }
                }
            }
        }
    ...
    int VND_Intra(vector<int>& rota, int custo_anterior){
        ...
        while(k < 5){
            switch(k){
                ...
                case 2: novo_custo = VND_Intra_ShiftN(rota, custo_anterior); break;
                case 3: novo_custo = VND_Intra_Reinsertion(rota, custo_anterior); break;
                case 4: novo_custo = VND_Intra_Inversion(rota, custo_anterior); break;
            }
    ...
    int VND_Inter(vector<int>& rota1, vector<int>& rota2, int custo_anterior){
        ...
        while(k < 6){
            ...
            switch(k){
                ...
                case 4: novo_custo = VND_Inter_Crossover(rota1, rota2, custo_anterior); break;
                case 5: novo_custo = VND_Inter_NOpt1(rota1, rota2, custo_anterior); break;
            }
