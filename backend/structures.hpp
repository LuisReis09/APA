...
public:
    ...
    {
        if (indice <= 0)
            throw runtime_error("Tentativa de insercao em indice negativo ou na garagem (invalido)");
        if (indice >= (this->estacoes.size() - 1))
            indice = this->estacoes.size() - 2;
    ...
    void RemoveAt(int indice)
    {
        ...
        // custo na direcao 1
        int custo = p.matriz_custo[this->estacoes[indice - 1]][this->estacoes[indice + 1]] - p.matriz_custo[this->estacoes[indice - 1]][this->estacoes[indice]] + p.matriz_custo[this->estacoes[indice]][this->estacoes[indice + 1]];
        this->custo_total_d1 += custo;
        // custo na direcao 2
        custo = p.matriz_custo[this->estacoes[indice + 1]][this->estacoes[indice - 1]] - p.matriz_custo[this->estacoes[indice + 1]][this->estacoes[indice]] + p.matriz_custo[this->estacoes[indice]][this->estacoes[indice - 1]];
...
public:
    ...
    int veiculos_usados;
    int veiculos_disponiveis;
    long long tempo_exec;
    vector<vector<int>> rotas;

    Solucao() : custo_total(0), veiculos_usados(0), veiculos_disponiveis(0), tempo_exec(0) {}
